...
list_showTimes_dance8 <- map(dance[[8]]$showInfo, ~.x$time)
list_showLocation_dance8 <- map(dance[[8]]$showInfo, ~.x$location)
```
...
levels(region) %>% 
  ...
  str_replace(.,"台中","中部") ->
  levels(region) # %>%只能串接到函數裡面當input，無法直接當作一個獨立物件
```
...
```{r}
population %>% names(sort(.)) # 只能串接到一個函數，不能串接到疊代函數
```
...
map(dataList, is.numeric) -> results # p.s
unlist(results) -> pick_numeric # name vector
