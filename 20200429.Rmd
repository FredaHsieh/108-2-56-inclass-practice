...
pick_afternoonRoutes <-
  stringr::str_detect(
    ...
    "下午"
  ) # 剛剛雖然把它改成factor(會用數字儲存)，但str_detect會根據螢幕顯示出的字串來運作
...
str_detect(fruit, "a")
str_detect(fruit, "^a") # starting with # 字元規律描述 Regular expression
...
str_detect(fruit, "^p", negate = TRUE) # !str_detect(fruit, "^p"）
```
