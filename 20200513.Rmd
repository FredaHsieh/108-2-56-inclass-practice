...
new_case <- sort(case, decreasing = T)
new_levels <- names(new_case)
print(new_case)
covid19$`縣市` <- factor(covid19$`縣市`, levels = new_levels)
levels(covid19$`縣市`)
```
...
# 整併
north_regex <- "宜蘭縣|基隆市|[台臺]北市|新北市|桃園市|新竹市|新竹縣" # [台臺]北市 可視為一字元strip 相當於 台|臺北市
# 整個regex為一組字組strip
...
# 創造10歲間距的所有可能
start <- seq(5, 55, by=10) # 由5到55，每個值差10的數值向量 # seq(begin, end, by = )
end <- seq(14, 64, by=10)
middleLevels <- 
  rep(
    paste0(start,"-",end), 
    each=2) # 每個新間距要2個 # rep(x, each = ) 每個元素重複幾次
...
```{r}
factor2[[6]] <- "差"
factor2 # 元素6變NA # 因為無法從levels中找到 差 這個類別
```
...
garbageRoutes$route <- factor(str_extract(garbageRoutes$linename, route_regex))
garbageRoutes$route

stop_amount <- table(garbageRoutes$route)
stop_amount <- sort(stop_amount)
new_levels <- names(stop_amount)
garbageRoutes$route <- factor(garbageRoutes$route, levels = new_levels)
levels(garbageRoutes$route)

time_regex <- "(?<=線)[:graph:]+"
garbageRoutes$time <- str_extract(garbageRoutes$linename, time_regex)
